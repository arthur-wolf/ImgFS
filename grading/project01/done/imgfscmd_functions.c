...
int help(int useless _unused, char** useless_too _unused)
{
    ...
    puts("                                  default value is 128"); // grader : hardcoded strings -0.5
...
int do_create_cmd(int argc, char** argv)
{
    ...
    strncpy(imgfsFile.header.name, imgfs_filename, MAX_IMGFS_NAME); // grader : unnecessary, already copied in do_create -1
...
int do_delete_cmd(int argc, char** argv)
{
    ...
    if (do_open(imgFS_filename, "rb+", &imgfs_file) != ERR_NONE) {
        return ERR_IO; // grader: reducing error type -1 (do_open may return ERR_OUT_OF_MEMORY)
    }
